...
@section('main-content')
            @if(isset($client) && $client)
                <div class="gap_top alert alert-info mb-3">
                    <strong data-en="Client:" data-fr="Client :">Client:</strong> {{ $client->client_name ?? 'N/A' }}<br>
                    <strong data-en="Email:" data-fr="Courriel :">Email:</strong> {{ $client->client_email ?? 'N/A' }}<br>
                    <strong data-en="Status:" data-fr="Statut :">Status:</strong> {{ $client->client_status ?? 'N/A' }}
                </div>
            @endif
            @if(empty($columns))
            ...
            <div class="pagination-controls d-flex justify-content-center align-items-center mt-3 gap-3">
                <button id="prev-page-btn" class="btn btn-outline-primary" onclick="changePage(currentPage - 1, currentCategoryId)" {{ request('page', 1) <= 1 ? 'disabled' : '' }}>
                    Previous
                </button>
                ...
                <button id="next-page-btn" class="btn btn-outline-primary" onclick="changePage(currentPage + 1, currentCategoryId)" {{ request('page', 1) >= $tableData->lastPage() ? 'disabled' : '' }}>
                    Next
                </button>
            </div>
            @endif
